...
FROM --platform=$TARGETPLATFORM registry.cn-hangzhou.aliyuncs.com/acs/alinux:3-update
RUN sudo yum install -y automake gcc-c++ git libcurl-devel libxml2-devel \
    fuse3-devel make openssl-devel rpm-build rubygems ruby ruby-devel unzip
RUN ruby -v 
RUN gem -v  
